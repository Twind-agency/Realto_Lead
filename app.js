...
const messageKindOptions = [
  { value: "first", label: "Primo messaggio" },
  { value: "followup", label: "Follow-up" },
  { value: "lastcall", label: "Ultima chiamata" },
  { value: "postRegistration", label: "Post registrazione" },
];

const messageKindByLeadId = {};
...
function applySharedEndpointFromUrl() {
...
async function loadRemoteConfig() {
  ...
  try {
    const response = await fetch(`${settings.endpoint}?action=getConfig`);
    const data = await response.json();
    if (!data.ok || !data.config) return;

    settings = {
      ...settings,
      firstTemplate: normalizeFirstTemplate(data.config.firstTemplate || settings.firstTemplate),
      followUpTemplate: normalizeFollowUpTemplate(data.config.followUpTemplate || settings.followUpTemplate),
      lastCallTemplate: normalizeLastCallTemplate(data.config.lastCallTemplate || settings.lastCallTemplate),
      postRegistrationTemplate: normalizePostRegistrationTemplate(data.config.postRegistrationTemplate || settings.postRegistrationTemplate),
      statuses: defaultStatuses,
      dynamicFields: normalizeDynamicFields(data.config.dynamicFields || settings.dynamicFields, defaultDynamicFields),
    };
    persistSettings();
  } catch {
    els.syncHelp.textContent = "Configurazione remota non caricata";
  }
}
...
function getLeadTimestamp(value) {
  const timestamp = new Date(value).getTime();
  return Number.isNaN(timestamp) ? 0 : timestamp;
}

function getContactedStatus() {
  const alreadyContacted = settings.statuses.find(
    (status) => status.toLowerCase().includes("contatt") && !status.toLowerCase().includes("contattare"),
  );
  return alreadyContacted || settings.statuses.find((status) => status.toLowerCase().includes("contatt")) || settings.statuses[2] || settings.statuses[0];
}
...
function filteredLeads() {
  ...
  return state.leads
    ...
    .sort((a, b) => getLeadTimestamp(b.createdAt) - getLeadTimestamp(a.createdAt));
}
...
function renderList() {
  ...
  leads.forEach((lead) => {
    ...
    const messageSelect = node.querySelector(".message-select");
    ...
    const selectedKind = messageKindByLeadId[lead.id] || getDefaultMessageKind(lead);
    messageSelect.innerHTML = messageKindOptions
      .map((option) => `<option value="${option.value}" ${option.value === selectedKind ? "selected" : ""}>${escapeHtml(option.label)}</option>`)
      .join("");
    quickWhatsapp.href = buildWhatsappUrl(lead, selectedKind);
    ...
    messageSelect.addEventListener("change", () => {
      messageKindByLeadId[lead.id] = messageSelect.value;
      quickWhatsapp.href = buildWhatsappUrl(lead, messageSelect.value);
    });
    quickWhatsapp.addEventListener("click", async (event) => {
      event.preventDefault();
      const selectedMessageKind = messageSelect.value;
      const url = buildWhatsappUrl(lead, selectedMessageKind);
      window.open(url, "_blank", "noopener");
      if (Number(lead.whatsappCount || 0) === 0 && selectedMessageKind !== "postRegistration" && !isRegisteredLead(lead)) {
        markLeadContactedFromCard(lead.id, card).catch(() => {});
      } else {
        registerWhatsappFollowUpFromCard(lead.id, card).catch(() => {});
      }
    });
...
function getDefaultMessageKind(lead) {
  if (isRegisteredLead(lead)) return "postRegistration";
  const count = Number(lead.whatsappCount || 0);
  if (count === 0) return "first";
  if (count === 1) return "followup";
  return "lastcall";
}

function isRegisteredLead(lead) {
  const status = normalizeVariableKey(lead?.status);
  return status === "fissato colloquio" || status === "registrato";
}
...
function buildWhatsappUrl(lead, forcedKind) {
  const kind = forcedKind || getDefaultMessageKind(lead);
  ...
  // Il client desktop di WhatsApp corrompe le emoji quando il browser gli passa
  // il link wa.me tramite l'handoff del protocollo. Passando direttamente da
  // web.whatsapp.com si resta nel browser e le emoji arrivano integre.
  const whatsappUrl = isMobileDevice()
    ? new URL(`https://wa.me/${phone}`)
    : new URL(`https://web.whatsapp.com/send?phone=${phone}`);
  whatsappUrl.searchParams.set("text", normalizeWhatsappText(text));
  return whatsappUrl.toString();
}

function getWhatsappTemplate(kind) {
  if (kind === "postRegistration") return settings.postRegistrationTemplate;
  if (kind === "lastcall") return settings.lastCallTemplate;
  if (kind === "followup") return settings.followUpTemplate;
  return settings.firstTemplate;
}

function normalizeWhatsappText(value) {
  const text = String(value || "").replace(/\r\n?/g, "\n").normalize("NFC");
  return typeof text.toWellFormed === "function" ? text.toWellFormed() : replaceInvalidSurrogates(text);
}
...
function replaceInvalidSurrogates(value) {
  return value.replace(/[\uD800-\uDFFF]/g, (character, index) => {
    ...
    const isValidHigh = code >= 0xD800 && code <= 0xDBFF && next >= 0xDC00 && next <= 0xDFFF;
    const isValidLow = code >= 0xDC00 && code <= 0xDFFF && previous >= 0xD800 && previous <= 0xDBFF;
    return isValidHigh || isValidLow ? character : "";
  });
}
...
function getWhatsappVariableValue(lead, key) {
  ...
  if (normalizedKey === "link video" || normalizedKey === "linkvideo") {
    return settings.linkVideo || "{{link_video}}";
  }
  ...
  if (settings.endpoint) {
    try {
      ...
      const params = new URLSearchParams({
        action: "updateLead",
        payload: JSON.stringify({ lead: buildSyncLead(lead) }),
      });
...
function buildSyncLead(lead) {
  return {
    id: lead.id || "",
    rowNumber: lead.rowNumber || lead._rowNumber || "",
    status: lead.status || settings.statuses[0] || "Nuovo",
    notes: lead.notes || "",
